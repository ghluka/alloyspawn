...
public class PrimePlayerFinishesUsingItemProcedure {
	public static void execute(Entity entity) {
		...
		if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
			_entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, (int) (5 * 20), 1));
		if (Math.random() < 0.5) {
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(AlloyspawnModMobEffects.MERCURY_POISONING.get(), (int) (36000 * 20), 0));
		}
	}
}
